...
public class TempCurrentPDFOperation {
    static {
...
//    static final String pngOutPath = "D:\\LDA\\result_output\\pngOutPath\\";
     ...
     * @Date: 2020/2/11
     */
    public static void showCurrentLine(File file, List<Rectangle> list, int page) throws IOException {
        //原文件绝对目录
...
//        String Config=res+"conf.txt";
        ...
        try {
            image = renderer.renderImage(page, 3f);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for(Rectangle rectangle:list) {
            g.drawRect((int) (rectangle.x / r), (int) (rectangle.y / r),
                    (int) (rectangle.width / r), (int) (rectangle.height / r));
        }


        FileOutputStream out = null;//输出图片的地址
        try {
            out = new FileOutputStream(res + page + "dpi"+Config.dip+".png");
...
//    static final String tempSplitPath = "D:\\LDA\\runnning_output\\tempSplitPng\\";//本地
    ...
    public static List<Rectangle> getPageRectangle(File pdfFile, int pageNum) throws IOException {
...
//        deleteFiles(tempSplitPath);
        ...
        list.add(0, rec);
        image = inputStream2Mat(returnPath + "temp_" + pageNum + ".png");
